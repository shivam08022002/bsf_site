...
export default function TournamentMatches({ role, logout, supportedSports, isSmallScreen }) {

    const { state } = useLocation();
    const { seriesId } = state ? state : "";
    console.log("tournament matches", seriesId);
    ...
    const fetchCricketMatches = async () => {
        api
            .get(`${getLiveGames}` + selectedSport + '&seriesId=' + seriesId)
            .then(res => {
                console.log("series matches", res);
            ...
            <div className="inplay-sports-button-matches-card-container">
                ...
                <div className="inplay-matches-card-container">
                    {matches && matches.map((match, index) => (
                        <div key={index} className="inplay-match-card-container" onClick={(e) => handleMatchClick(e, match)}><MatchesCardView id={match.id} match={match} status={match.matchStatus} isSmallScreen={isSmallScreen} /></div>
                    ))}
